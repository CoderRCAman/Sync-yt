...
socket.emit('connect-user', { 'username': username, 'serverId': serverId, 'host': true })
...
videoid = sessionStorage.getItem('videoId') ? sessionStorage.getItem('videoId') : ''; //will retreive information when page refreshes 
...
function findVideoId(url) {
    ...
    while (len) {
        --len;
        if (url[len] === '/' || url[len] === '=') return result.split('').reduce((rev, currentChar) => currentChar + rev, '');
        result += url[len];
    }
    return result
}
videoURLForm.addEventListener("submit", (e) => {
    ...
    //  console.log(videoid);
    socket.emit('change-url', { 'username': username, 'videoId': videoid, 'serverId': serverId })
    sessionStorage.setItem('videoId', videoid);
})

socket.on('chat request', (data) => {
    const chat = sessionStorage.getItem('messages') === null ? [] : JSON.parse(sessionStorage.getItem('messages'))
    //  console.log('receip')
...
window.addEventListener('load', (e) => {
    //load all messages  
    const messages = sessionStorage.getItem('messages') ? JSON.parse(sessionStorage.getItem('messages')) : [];
...
socket.on('message', (msg) => {
    if (msg.serverId === serverId) {
        ...
        var messages = JSON.parse(sessionStorage.getItem('messages')) ? JSON.parse(sessionStorage.getItem('messages')) : [];
        messages.push(msg);
        sessionStorage.setItem('messages', JSON.stringify(messages));
        outputMsg(msg)
